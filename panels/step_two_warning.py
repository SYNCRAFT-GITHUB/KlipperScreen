...
def create_panel(*args):
    return StepTwoWarning(*args)

class StepTwoWarning(ScreenPanel):

    def __init__(self, screen, title):

        super().__init__(screen, title)
        self.menu = ['step_two_warning']

        self.image = self._gtk.Image("network", self._gtk.content_width * .7, self._gtk.content_height * .7)
        
        self.labels['text'] = Gtk.Label(_("Do not turn off Syncraft during the procedure, and keep a stable internet connection."))
        ...
        self.buttons['CONTINUE'].connect("clicked", self.menu_item_clicked, "step_three_warning", {
            "name": _("Fix"),
            "panel": "step_three_warning"
        })
